...
The coordinated server MUST register the five Steam query tools—`steam_get_library`, `steam_search_library`, `steam_get_game`, `steam_get_recent_games`, and `steam_get_library_stats`—and MUST also register all six gaming tracker tools; each MUST validate arguments and return normalized output. (Previously: discovery required exactly five total tools.)
