...
import type { HomeTaskSummary } from "./views/home-view.js";

export type TaskApi = DashboardApi &
  Required<Pick<DashboardApi, "getTasks" | "getTask" | "cancelTask">>;
...
export function useTaskState(api: TaskApi): TaskState;
export function useTaskState(api: TaskApi | undefined): TaskState | undefined;
export function useTaskState(api: TaskApi | undefined): TaskState | undefined {
  ...
  const nextTaskRequestVersion = useCallback((id: string): number => {
    const version = (taskRequestVersions.current.get(id) ?? 0) + 1;
    taskRequestVersions.current.set(id, version);
    return version;
  }, []);

  const applyTaskIfCurrent = useCallback((task: LocalTask, version: number) => {
    setTasks((current) => {
  ...
  useEffect(() => {
    if (api === undefined || !tasks.some(isActiveTask)) return;
    const poll = async () => {
  ...
  }, [api, applyTaskIfCurrent, nextTaskRequestVersion, tasks]);
  ...
  const cancel = useCallback(
    ...
    [api, applyTaskIfCurrent, nextTaskRequestVersion],
  );
